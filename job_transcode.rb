...
class Job_transcode
  ...
  def start
    ...
    if !Dir.glob('F:/Transcoder/staging/*.mp4').empty?
      ...
      Dir['F:/Transcoder/staging/*.mp4'].each do |f|
        ...
        if File.exists?("F:/Transcoder/staging/#{file_name}.mp4") && File.exist?("F:/Transcoder/staging/#{xml}")
          ...
          puts ''
          puts "#{time} Processing Task #{task_id}"
          puts ''
          ...
          puts ''
          puts "#{time} Task ID(#{task_id}) Conform started"
          puts ''

          system("#{conform}")

          puts ''
          puts "#{time} Task ID(#{task_id}) Conform complete"
          puts ''
